...
@section('content') 
          <div class="card card-primary card-outline">
            <div class="card-header with-border">
              <h3 class="card-title">{{ __('user.update_user') }}</h3>
            </div>
            <!-- /.card-header -->
            <!-- form start -->
			...
			<input name="_method" type="hidden" value="PATCH">
              <div class="card-body">
                ...
                <div class="form-group">
              	...
              	<div class="form-group">
               		<label for="email">{{ __('user.email') }}<span style="color:red">*</span></label><br>
               		{{ $user->email }}
                </div>
                
                <div class="form-group">
               		<label for="is_active">{{ __('user.is_active') }}<span style="color:red">*</span></label>
                  <select class="form-control{{ $errors->has('is_active') ? ' is-invalid' : '' }}" name="is_active" required>
                    <option value='Y' @if ($user->is_active == 'Y') selected @endif>YES</option>
                    <option value='N' @if ($user->is_active == 'N') selected @endif>NO</option>
                  </select>     
                ...
                <div class="form-group">
               		<label for="locked">{{ __('user.locked') }}<span style="color:red">*</span></label>
                  <select class="form-control{{ $errors->has('locked') ? ' is-invalid' : '' }}" name="locked" required>
                    <option value='Y' @if ($user->locked == 'Y') selected @endif>YES</option>
                    <option value='N' @if ($user->locked == 'N') selected @endif>NO</option>
                  </select>     
              	...
              	<div class="form-group">
               		<label for="role">{{ __('user.role') }}<span style="color:red">*</span></label>
                   	@php
                   	...
                   	@endphp                 	
					{{ Form::select('role_id', $list, $role_id, ['class' => 'form-control'.($errors->has('role') ? ' is-invalid' : '')]) }}
                ...
                <div class="form-group">
              	...
              	<div class="form-group">
               		<label for="origin_city">{{ __('competition.city') }}</label>
						...
						<input class='form-control city' type='hidden' id='city_id' name='city_id' value="{{($user->profile&&$user->profile->city?$user->profile->city_id:'')}}"/>
                    	@if ($errors->has('city'))
                			<span class="error invalid-feedback">
